angular.module('App', ['ngSanitize','ui.bootstrap','ngFileUpload']).controller('LoginCtrl',function ($scope,$http,$window) {
...
function ColorPassword(pass) {
  ...
  $scope.Registeruser = function()
  {
	...
	else
	{
				$http({
					method  : 'POST',
					url     : 'http://localhost:8070/api/Registeruser/',
 ...
 $scope.AuthUser = function()
  {
	if(!$scope.user || ($scope.user.mobile =="" || !$scope.user.mobile)|| ($scope.user.password =="" || !$scope.user.password))
	{
		swal({
			  type: 'error',
			  title: 'Oops...',
	...
	else
	{
				$http({
					method  : 'POST',
					url     : 'http://localhost:8070/api/Authuser/',
					data    : $scope.user,
					headers : {'Content-Type': 'application/json'} 
					}).then(function(response) {
					...
					{	
						$window.localStorage['token'] = JSON.stringify(response.data.data[0].token);
						$window.localStorage['lastname'] = JSON.stringify(response.data.data[0].lastname);
						$window.localStorage['firstname'] = JSON.stringify(response.data.data[0].firstname);
						$window.localStorage['id'] = JSON.stringify(response.data.data[0]._id);
						$window.localStorage['email'] = JSON.stringify(response.data.data[0].email);
