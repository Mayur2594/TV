...
var userData = mongoose.model('usersDetails');
...
var opts = {
  ...
  exports.Registeruser = function (req, res) {
	...
	()=>{
				 var UsersDetails = new userData(req.body);
...
exports.Authuser = function (req, res) {
	...
	mongoose.connect(dblink,opts).then(
	()=>{
		userData.find({'mobile':req.body.mobile,'password':req.body.password},{'firstname':1,'lastname':1,'email':1}).lean().exec(function (err, result) {
			if(err)
				{
					res.json({status:0,message:'Something went wrong, please try again'});
					 mongoose.disconnect();
